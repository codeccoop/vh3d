...
Geometry.prototype.scale = function () {
  for (let feat of this.json.features) {
    ...
    if (type === "Point") {
      feat.geometry.coordinates = [this.xScale(feat.geometry.coordinates[0]), this.yScale(feat.geometry.coordinates[1])];
...
Geometry.prototype.buildUVSurface = function (map, geometry, xSegments, ySegments, z) {
  ...
  const plane = new THREE.PlaneGeometry(lngs[1] - lngs[0], lats[1] - lats[0], 1, 1);
