    ...
    this.loadGltfs(() => {
      campus.load().then(campus => {
        ...
        if (canvas.clientWidth < canvas.clientHeight) {
          this.scene.state.scale = Math.min(1, (campus.yScale(1) - campus.yScale(0)) / 1);
        } else {
          this.scene.state.scale = Math.min(1, (campus.xScale(1) - campus.xScale(0)) / 1);
        }

        if (this.scene.legoPiece) {
          const args = Array.apply(null, Array(3)).map(d => this.scene.state.scale);
