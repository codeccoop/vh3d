...
class Scene extends THREE.Scene {
  constructor(canvas, mode) {
    ...
    this.$on("bbox:update", ev => {
      ...
      if (canvas.clientWidth < canvas.clientHeight) {
        ...
        const yRange = [bbox.lats[0] - delta * percent * 0.5, bbox.lats[1] + delta * percent * 0.5];
        ...
        const xRange = [bbox.lngs[0] - delta * percent * 0.5, bbox.lngs[1] + delta * percent * 0.5];
  ...
  onControlInit(ev) {
    ...
    setTimeout(() => this.control.dispatchEvent({
      type: "change"
    }), 100);
  }
  ...
  initPosition() {
    const rescaledOrigin = [this.xScale(origin[0]), this.yScale(origin[1])]; // const rescaledOrigin = [this.xScale(target[0]), this.yScale(target[1])];
    ...
    if (this.exitLabel) {
      this.exitLabel.position.set(...rescaledOrigin, 0.5);
      this.exitLabel.position.x += 15;
      this.exitLabel.position.y -= 0;
      this.targetLabel.position.set(...rescaledTarget, 0.5);
